...
const uint8_t PROGMEM M[] = {98, 97, 96, 77, 78, 79, 76, 75, 74, 55, 56, 57, 54, 53, 52, 33, 34, 35, 22, 23, 24, 11, 12, 13, 10, 9, 8, 14, 25, 26, 36, 37, 38, 50, 49, 48, 60, 28, 29, 18, 2, 1, 0, 19, 20, 21, 30, 31, 32, 39, 40, 41, 42, 43, 46, 45, 44, 63, 64, 65, 68, 67, 66, 85, 86, 87, 90, 89, 88, 100};
...
// function to handle any turn-on given a buffer (max size of NUM_LEDS)
void processLEDBuffer(uint8_t color_mode) {
  if (buf_cnt > 0) {

    unsigned int time_delay = 2000 / buf_cnt;
    //unsigned int time_delay = 2;
    
    unsigned int b_rainbow = random(0, 2);

    Serial.println(b_rainbow);

    
    uint8_t values[buf_cnt];
    uint8_t hues[buf_cnt];
    ...
    // basic, just fade each one on, possibly with rainbow
    if (color_mode == 0) {
      for (int i = 0; i < buf_cnt; i++) {
        leds[LED_BUFFER[i]] = CHSV( hues[i], gSat, 0);
      }
  
      while (values[buf_cnt-1] < 255) {
        for (int i = 0; i < fade_cnt; i++) {
          ...
          leds[LED_BUFFER[i]] = CHSV( hues[i], gSat, values[i]);
        }
        
        if (((millis() - time_last) > time_delay) & (fade_cnt < buf_cnt)) {
          time_last = millis();
          fade_cnt ++;
        }
    ...
    if (color_mode == 1) {
      ...
      turnAllOffAtRandom(true, LED_BUFFER); // also blocking
      delay(3000);
    }
...
void fadeInBy(uint8_t led_pos, uint8_t by) {
  
}
void addToBuffer(uint8_t * seq) {
  uint8_t cnt = 0;
  while ((*(seq + cnt) != 100) & (cnt < NUM_LEDS + 1)) {
    addToBuffer(*(seq + cnt));
    cnt++;
  }
}

void addToBuffer(uint8_t pos) {
  LED_BUFFER[buf_cnt] = pos;
  buf_cnt++;
  if (buf_cnt >= NUM_LEDS) {
    buf_cnt = NUM_LEDS;
  }
...
void turnOn(Word this_word) {
  turnOn(this_word, false);
}

void turnOn(Word this_word, bool b_buffer) {
  for (int j = 0; j < 6; j++) {
    if (this_word.cols[j] == 100) {
      return;
    }
    if (b_buffer) {
      addToBuffer(letters[this_word.row][this_word.cols[j]]);
    } else {
...
void stripeDownRainbow() {
  stripeDownRainbow(false);
}

void stripeDownRainbow(bool b_fade) {
  uint8_t hue = 0;
...
void turnOnAtRandom(uint8_t * seq) {
  ...
  uint8_t refHue = random(0, 255);
  for (int i = 0; i < NUM_LEDS; i++) {
    uint8_t hue = refHue +  dHue * sequence[i];
...
void doSecretThing() {
  ...
  uint8_t color_mode = random(0,19) / 10;

  Serial.println(color_mode);
  processLEDBuffer(color_mode);
