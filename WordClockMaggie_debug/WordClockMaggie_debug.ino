...
uint8_t secret_sequence[9] = {15, 16, 17, 20, 85, 86, 98, 97, 100}; // hard-coded secret sequence, these are LED-positions
uint8_t come_on_eileen[] = {8, 13, 7, 16, 76, 75, 29, 39, 48, 62, 70, 84, 100};
...
void turnAllOffAtRandom() {
  turnAllOffAtRandom(false);
}

void turnAllOffAtRandom(bool b_secret_sequence) {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    if (b_secret_sequence) {
      ...
      if (!checkInSequence(sequence[i], M)) {
    ...
    if (state != but_state[n]) {
      ...
      if (state == HIGH) {
        ...
        if (n == 2) {
          ...
          turnAllOffAtRandom(true); // also blocking
          delay(3000);
        }
        if (n == 3) {
          changeColorState();
