...
#if !defined(__AVR)
  //drawCornerTest();
  showFontCustom(&FreeMonoBold12pt7b);
  drawGraph();
  drawBar();
  delay(10000);
  delay(2000);
  //showFontCustom(&FreeMonoBold9pt7b);
...
void drawGraph() {
  //display.setCursor(58+(19+5)*4+100,0);
  int ytop = 58+(19+5)*4+100;
  for (int i = 1; i<390; i++) {
    display.drawLine(i+5,ytop-history[i-1],i+1+5,ytop-history[i],0);
  }
  display.drawLine(5,ytop,395,ytop,0);
  display.setFont();
  for (int i=0; i<400; i=i+50) {
    display.drawLine(i,ytop,i,ytop+5,0);
    display.setCursor(i,ytop+5);
    display.print(i);
  }
}
...
void showFontCustom(const GFXfont* f)
{
  display.fillScreen(GxEPD_WHITE);
  display.setTextColor(GxEPD_BLACK);
  display.setFont(f);
  display.setCursor(0, 0);
  display.println();
  display.println("Tarka System monitor v0.1");
  display.println();
  display.println("Batteries: [          ] 79%");
  display.println("Tank 1:!   [          ] 60%");
  display.println("Tank 2:    [          ] 100%");
  display.println("Tank 3:    [          ] 0%");
  display.setFont(&FreeMono9pt7b);
  display.setCursor(0,280);
  //display.println("   Systems: Operational");
  display.println("   Last update: 11/30/2019 10:55");
#if defined(HAS_RED_COLOR)
  display.setTextColor(GxEPD_RED);
#endif
  //display.update();
